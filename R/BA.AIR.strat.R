#' Get the latest position suggestion for BA~AIRBUS cointegration trading strategy
#'
...
#'
#' @examples
#' BA.AIR.strat(fromdate)
BA.AIR.strat <- function(fromdate="2015-01-01"){
  ...
  todaymean = tail(rolling$mean,n=1)
  todayspread = tail(rolling$Spread,n=1)
  todaylower = tail(rolling$lower_bound,n=1)
  todayhigher = tail(rolling$higher_bound,n=1)

  strategy <- ifelse(todayspread>=todayhigher,"Sell Short",
                     ifelse(todayspread==todaymean,"exit",
                            ifelse(todayspread<=todaylower,"Buy Long","Stay")))
